  ...
  displayData(stock: string) {
    ...
    this.httpClient.get(url, {headers: headers}).subscribe(
        (data) => {
          ...
          const price = '<h2>Price</h2><span style="color:black;"> Market Price: </span>' + data['price']['regularMarketPrice']['fmt'] + '<br>' +
          ...
          const finance = '<h2>Financial Data</h2><span style="color:black;"> Profit Margin: </span>' + data['financialData']['profitMargins']['fmt'] + '<br>' +
    ...
    this.httpClient.get(newsUrl, {headers: headers}).subscribe(
      (data) => {
